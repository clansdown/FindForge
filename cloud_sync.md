...
### 4.2 `src/auth.ts` — Full source (adapt to your Clerk version)

> **Clerk version note:** The reference implementation uses Clerk v3 (`@clerk/clerk-js`
> v3.x). Clerk v4+ uses named exports and a different `clerk.load()` API. Check your
 ...
 * Loads the @clerk/ui bundle for embedded sign-in/sign-up components.
...
export function isSignedIn(): boolean {
    return (clerkInstance as Record<string, unknown> | null)?.isSignedIn as boolean | undefined ?? false;
}
...
The sync journal is an event-sourced crash-recovery system (~255 lines).
It has no app-specific code.
...
| Function | Purpose |
|----------|---------|
| `init(): Promise<void>` | Reads checkpoint, loads journal, replays crash recovery from localStorage, determines next ID. Uses singleton init pattern to prevent concurrent initialization. |
...
| `drainQueue(): Promise<void>` | Persists dirty entries to the active journal file. Clears localStorage recovery data after successful write. |
...
| `truncateJournalEntriesBefore(syncedId: number): Promise<void>` | Called after successful `syncToCloud` to remove synced entries from the journal, preventing unbounded growth. |

**Crash recovery features:**
- **`beforeunload` handler:** Saves dirty entries to `localStorage` on tab close/crash so they are not lost during the 5-second debounce window. Replayed on next `init()`.
- **Journal read cache:** Parsed entries are cached by journal name to avoid redundant file reads and JSON parsing.
- **Corruption recovery:** If the current journal is unreadable, falls back to the alternate journal.
- **Rotation cleanup:** Deletes the old journal file after a confirmed rotation to the new journal.
- **Cross-journal reads:** `getJournalEntriesSince()` reads entries from both journals when `lastId` predates the current journal, ensuring no entries are missed after rotation.

**These files are NEVER synced to cloud.** They are local crash-recovery journals
only. The sync engine's `syncToCloud()` calls `drainQueue()` before
building the upload batch, and `truncateJournalEntriesBefore()` after a successful sync.
...
// Runtime preference read from /cloud/preferences/cloudSyncDeleteRemote
...
| `enableCloudSync(): Promise<void>` | Toggle on. Triggers the initial sync |
| `disableCloudSync(): Promise<void>` | Toggle off |
| `syncToCloud(): Promise<void>` | Upload dirty journal entries. Truncates journal after successful sync. |
...
| `syncResetThenPull(): Promise<void>` | Delete manifest, full re-sync from cloud |
...
| `triggerCompleteResync(): Promise<void>` | Full re-sync |
| `computeHash(content: string \| ArrayBuffer): string` | MD5 hash for B2 ETag compatibility. Handles both text and binary content. |
| `getCloudSyncState(): CloudSyncState` | Returns current sync state (enabled, isSyncing, lastSyncTime, lastSyncError, syncProgress) |
| `setConflictResolver(resolver): void` | Register a custom conflict resolution function |
...
3. If enabled and journal is empty (first-run), runs `syncBothWays()` (push local first, then pull). An HTML modal template (`initial-sync-modal-template`) exists in `index.html` for a user-facing upload-vs-download choice in future UX work.
4. Otherwise does `syncToCloud()` to push any local changes
5. Starts the 1-hour `syncFromCloud()` timer
6. Sync state is exposed via `getCloudSyncState()` for reactive UI binding
...
3. Runs the sync function, passing a `ctx: { changedPaths: string[] }` collector
4. In the `finally` block: releases the lock, fires `syncReloadCallback(changedPaths)`

Each sync function pushes downloaded/modified paths into `ctx.changedPaths`.
The callback is fired centrally by `withSyncLock` rather than by each function
individually. Functions that don't modify local files (like `syncToCloud`) simply
don't push paths, and the callback is not invoked.
...
**Default behavior:** Conflicts are auto-resolved as remote-wins for downloads
(`syncFromCloud`). Upload-oriented operations (`syncToCloud`) skip conflicting
files to avoid overwriting remote data.

**Custom resolver hook:** Register a `ConflictResolver` function via
`setConflictResolver(resolver)`. The function receives a `SyncConflict` and
returns `'local'` or `'remote'`. This hook is intended for future modal-based
resolution.
...
**Modal templates** exist in `index.html` for future UX wiring:
- `sync-conflict-modal-template` — per-file conflict resolution (pick local vs. remote)
...
**`.png` file conflicts** are auto-resolved as uploads (local-wins) to preserve
image quality. Binary files are detected via Content-Type header.
...
const isCredentialsPrefix = path === 'credentials' || path.startsWith('credentials/');
const cloudPrefix = isCredentialsPrefix ? CREDENTIALS_CLOUD_PREFIX : CLOUD_PREFIX;
const requestPath = isCredentialsPrefix
    ? (path.startsWith('credentials/') ? path.slice('credentials/'.length) : '')
    : path;
...
This pattern should be applied to every function that writes or deletes app data.
In FindForge Translate, this is applied across ~16 functions in `src/storage.ts`.
Every `saveSession()`, `saveSessionTranslation()`, `deleteSession()`,
`savePreference()`, `deletePreference()`, etc. follows it.
The number of instrumented functions depends on your app's data model.
...
- Manual sync button
- Sync status text (from `getCloudSyncState()`)
- Progress tracking: `getCloudSyncState()` returns `syncProgress: { current, total, phase } | null`. Phase strings are `'Uploading'` / `'Downloading'`. Set to `null` when idle. Bind reactively in your UI framework.
...
Body: raw file content (string or ArrayBuffer)
```
Returns: `200 OK` with `ETag` header. The local hash uses MD5 for B2 ETag compatibility. Binary files are hashed by converting their bytes to a binary string first.

### DOWNLOAD file
```
GET /{CLOUD_PREFIX}{path}
```
Returns: file content. Binary files (`.png`, `image/*` Content-Type) return `ArrayBuffer`; text files return `string`.

### DELETE file
```
DELETE /{CLOUD_PREFIX}{path}
```
Gated by the `shouldAllowCloudDeletions()` runtime preference (reads `/cloud/preferences/cloudSyncDeleteRemote`). In production, deletions should be disabled (preference set to `false`).
...
- **`ALLOW_CLOUD_DELETIONS` is a runtime preference.** Read from
  `/cloud/preferences/cloudSyncDeleteRemote` via `shouldAllowCloudDeletions()`.
  The `setDeleteRemoteOnLocalDelete()` function updates both the preference file
  and an in-memory cache. Keep disabled (`false`) in production.

- **`.png` conflicts are auto-resolved as uploads.** Binary files are treated as
  local-wins to preserve image quality. All other file types can use the
  `ConflictResolver` hook for resolution.

- **Journal is truncated after successful sync.** `syncToCloud()` calls
  `truncateJournalEntriesBefore()` after the manifest is saved, preventing
  unbounded journal growth and redundant re-processing.

- **Dirty entries survive crashes via `beforeunload`.** On tab close or crash,
  un-synced entries are saved to `localStorage` and replayed on next `init()`.
  This closes the 5-second debounce loss window.

- **Journal corruption recovery is automatic.** If the current journal is
  unreadable, the sync engine falls back to the alternate journal. Both journals
  are cross-read in `getJournalEntriesSince()` to prevent data loss after rotation.

- **Journal files are rotated when they reach 10,000 entries.** The old journal
  file is deleted after the rotation is confirmed. A fresh journal-A or journal-B
  file is created, and the checkpoint switches to the new journal.

- **Credential prefix detection handles trailing slash.** The credential path
  check accepts both `'credentials'` and `'credentials/'` forms, fixing a
  mismatch where `listRemoteFiles('credentials')` would use the wrong cloud prefix.

- **Sync progress is tracked via `STATE.syncProgress`.** Exposed through
  `getCloudSyncState()` with `{ current, total, phase }` fields. Phase strings
  are `'Uploading'` / `'Downloading'`. Set to `null` when idle.

- **Reactive UI replaces `ui.ts` module.** Instead of a centralized sync UI
  module, the current `STATE` object is exposed via `getCloudSyncState()` for
  reactive binding in the app framework (Svelte stores, React hooks, etc.).
...
| **Sync UI** | `index.html` + menu bar | Navbar button, settings tab (with progress tracking), modal templates |
